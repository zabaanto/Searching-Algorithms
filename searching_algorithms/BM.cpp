...
void bm_algorithm(string text, string pattern)
{
	...
	bm_alg_pattern_preprocess(pattern, map);  // first position of the alphabet's element in the pattern from its end
	...
	while (main_index < n)
	{
		...
		{
			main_index += m - control_index > m - (map[text[main_index]] + 1) ? m - control_index : m - (map[text[main_index]] + 1);  // shift by BM algorithm (at least trivial shift or more)
			control_index = m - 1;
		}
	}
...
void bm_alg_pattern_preprocess(string &pattern, map<char, int> &map)
{
	for (auto pair : map)
	{
		...
		while ((j > -1) && (pattern[j] != pair.first)) j--;
		map[pair.first] = j;
	}
}